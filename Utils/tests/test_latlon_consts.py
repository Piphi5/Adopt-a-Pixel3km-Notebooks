...
def test_latlon_consts(lat, lon, distance):
  ...
  assert haversine(point, west_point, unit=Unit.METERS) == pytest.approx(distance, 0.001)
  assert haversine(point, north_point, unit=Unit.METERS) ==pytest.approx(distance, 0.001)
